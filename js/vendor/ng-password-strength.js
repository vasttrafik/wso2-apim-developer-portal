(function() {
  ...
  angular.module('ngPasswordStrength', [])
    .directive('ngPasswordStrength',
      function() {
        return {
          template: '<div class="progress {{valueClass.outter}}"><div class="{{valueClass.inner}} {{innerClass}}" role="progressbar" aria-valuenow="{{value}}" aria-valuemin="0" aria-valuemax="100" ng-style="{width : ( value + \'%\' ) }"><span class="sr-only">{{value}}%</span>{{text}}</div></div>',
          restrict: 'A',
          scope: {
          ...
          link: function(scope /*, elem, attrs*/ ) {
            ...
            scope.$watch('pwd', function() {
              ...
              scope.valueClass = getClass(scope.value);
              if (scope.value === 100) {
                scope.text = 'Bäst säkerhet';
              } else if (scope.value > 75) {
                scope.text = 'Bra säkerhet';
              } else if (scope.value > 50) {
                scope.text = 'Ok säkerhet';
              } else if (scope.value > 1) {
                scope.text = 'Låg säkerhet';
              } else {
                scope.text = '';
              }
            });
